(function () {
  ...
  AB.load('projects').then(function (d) {
    ...
    // Mot o thong so de trong thi in ra "Not recorded" chu khong de trong: mot <dd> rong nhin
    // giong mot cho bi loi, con mot dong chu noi ro la khong co so lieu thi la mot cau tra loi.
    var facts = [['Year', a.year], ['Location', a.location], ['Client', a.client],
                 ['Scope', a.scope], ['Products', a.products.join(', ')]]
      .map(function (r) {
        return '<div class="ab-spec"><dt>' + AB.esc(r[0]) + '</dt><dd>' +
               AB.esc(r[1] || 'Not recorded') + '</dd></div>';
      }).join('');
    ...
    var others = d.albums.filter(function (x) { return x.id !== a.id; })
      .sort(function (x, y) { return (+y.year || 0) - (+x.year || 0); }).slice(0, 3).map(function (x) {
        return '<a class="ab-card" href="?id=' + encodeURIComponent(x.id) + '">' +
                 ...
                 '<p class="ab-card-spec">' + AB.esc(x.year ? x.year + ' · ' + x.location : x.location) + '</p></a>';
      }).join('');

    el.innerHTML =
      '<div class="ab-wrap">' +
        '<p class="ab-crumb"><a href="../">Projects</a>' +
          (a.year ? ' &nbsp;/&nbsp; ' + AB.esc(a.year) : '') + '</p>' +
